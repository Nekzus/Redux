...
--[[
for rob command: prob of failing is your networth / (their cash + your networth)
]]

commands:create({name = "pay",
	desc = "${paysSomeone}",
...
}):accept("transf", "transfer")

commands:create({name = "balance",
	desc = "${checksBalance}",
	usage = "",
...
}):accept("bal", "net", "networth")

commands:create({name = "work",
	desc = "${worksForMoney}",
	usage = "",
...
commands:create({name = "setcash",
	...
	func = function(data)
		...
		if value then
			local text = parseFormat("${cashSetSuccessful}", langList, member.tag, format("%s %s", symbol, affixNum(value)))
			local embed = replyEmbed(text, data.message, "ok")
...
}):accept("snet", "snetworth", "sn", "snw")

commands:create({name = "reseteconomy",
	...
	func = function(data)
		...
		local text = parseFormat("${serverEconomyReset}", langList)
		local embed = replyEmbed(text, data.message, "ok")
