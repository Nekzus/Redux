...
function saveAllData()
	if trackData and trackData.status == 1 then
		trackData:close()
	end

	if tempData and tempData.status == 1 then
		tempData:close()
	end
end
...
--[[
local saves = {}
saves.global = cache(db.load("global") or {})
saves.economy = cache(db.load("economy") or {})
saves.track = cache(db.load("track") or {})
saves.temp = cache(db.load("temp") or {})
--]]

--[[
print("OPEN")
local gdata = db("./saves/global/280874940171943936.bin", 0)
...
for gid, gdata in next, saves.global:raw() do
	local guild = db(format("./test/global/%s.bin", gid), 0)
...
for gid, gdata in next, saves.economy:raw() do
	local guild = db(format("./test/economy/%s.bin", gid), 0)
...
-- Temp
local temp = db("./saves/bot/temp.bin", 0)
...
-- Track
local temp = db("./saves/bot/track.bin", 0)
